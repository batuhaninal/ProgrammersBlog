...
namespace ProgrammersBlog.Services.Concrete
{
    public class CommentManager : ManagerBase, ICommentService
    {
        ...
        public async Task<IDataResult<CommentDto>> AddAsync(CommentAddDto commentAddDto)
        {
            ...
            if (article == null) return new DataResult<CommentDto>(ResultStatus.Error, Messages.Article.NotFound(false), null);
        ...
        {
            var comment = await UnitOfWork.Comments.GetAsync(c => c.Id == commentId, i => i.Article);
            if (comment !=null)
            {
                var article = comment.Article;
